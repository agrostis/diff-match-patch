...
(defun join (parts)
  (let ((parts* (remove-if #'empty parts)))
    (cond ((null parts*) #())
          ...
          (t (macrolet ((iter-joining ($type $parts)
                 `(iter outer (for part :in ,$parts)
                    (iter (for elem :in-sequence part)
                      (in outer (collect elem :result-type ,$type))))))
               (case (seq-type (car parts*))
                 ((vector) (iter-joining 'vector parts*))
                 ((string) (iter-joining 'string parts*))
                 (t (iter-joining 'list parts*))))))))
